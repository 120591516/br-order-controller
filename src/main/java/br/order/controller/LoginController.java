...
import br.order.realm.MyRealm;
import br.order.vo.BrUserVo;
/**
 * 登录的Controller
 * @ClassName: LoginController
 * @Description: TODO(登录的Controller)
 * @author adminis
 * @date 2016年9月13日 下午3:30:54
 *
 */
@Controller
	...
	public JSONObject login(@ApiParam(required=true,name="username",value="用户名")@RequestParam(value = "username", defaultValue = "", required = true)String username, 
		...
		try {
			...
			/*if (subject.hasRole("admin")) {
				logger.info("拥有【admin】角色。");
			} else {
				logger.info("不存在admin权限。");
			}*/
			BrUserVo brUser = commonController.getUserBySession();
//			RedisService.set(brUser.getUserId().toString(), JSON.toJSONString(brUser), 1800);
	...
	public JSONObject logout(){
		...
		return InterfaceResultUtil.toLogin();
	}
	 ...
	 * @return
	 */
	@ApiOperation(value="跳转到登陆",httpMethod="GET",response=JSONObject.class,notes="跳转到登陆")
